window.addEventListener('load', function () {
  ...
  fetch(url,settings)
    ...
    .then(data => {
      ...
      for(odontologo of data){
        ...
        let tr_id = 'doc'+odontologo.id;
        odontologoRow.id = tr_id;

        //por cada odontologo creamos un boton delete que agregaremos en cada fila para poder eliminar el misma
        //dicho boton invocara a la funcion de java script deleteByKey que se encargará
        //de llamar a la API para eliminar el odontologo
        ...
        let updateButton = '<button' +
                              ...
                              ' type="button" onclick="findBy('+odontologo.id+')" class="btn btn-info btn_id" data-bs-toggle="modal" data-bs-target="#updateModal">' +
                              odontologo.id +
                            '</button>';
  ...
  (function(){
    ...
    if (pathname == "/get_odontologos.html") {
      document.querySelector(".listar").addClass("active");
    }
  })
})